---
sidebar_position: 0
---

## Analytics
The Analytics dashboard provides a comprehensive visual overview of key human resource metrics. It helps management and HR personnel easily monitor and analyze data at a glance.

**How to View Analytics:**
1. Navigate to the **HR** -> **Analytics** module from the sidebar.
2. The visual dashboard will appear, showing various charts, summaries, and metrics based on current employee data.

![Analytics](/img/hr/analytics/analytics.png)

## View Details
The Analytics Dashboard contains multiple detailed widgets and charts that provide an in-depth view of various HR operations. Below is a breakdown of the highlighted sections on the dashboard:

![Analytics Dashboard Details](/img/hr/analytics/gemini.png)

**1. Quick Navigation Links**
A block of shortcuts to easily access core HR modules with one click:
- **EMPLOYEES DASHBOARD**, **DAILY ATTENDANCE**, **PAYROLL MANAGEMENT**, and **LEAVE MANAGEMENT**.

**2. Key Metrics Summary**
At the top right, numeric cards give a real-time snapshot of the workforce status:
- **Total Employees**: Complete headcount of the organization.
- **Total Presents**: Employees present today.
- **Total Absents**: Employees absent today.
- **Total On Leave**: Employees currently taking a leave.

**3. Employee Payroll Analysis**
This section offers financial insights regarding salary:
- Displays summaries like **Total Outstanding**, **Total Salary**, **Payable Days**, and **Total Generated**.
- Contains visual charts for tracking **Total Net Payroll** and the distribution of **Bonus & Deductions**.

**4. Employee Attendance Analysis**
- A graphical line chart displaying daily attendance trends over a continuous period, helping HR identify attendance patterns.

**5. Employee Asset Analysis**
- Analyzes the company assets (e.g., Laptops) issued to employees, cleanly visualized using a bar chart view.

**6. Employee Leave Request Analysis**
- A donut chart providing a percentage breakdown of leave applications categorized by their current status (**Approved**, **Pending**, and **Rejected**).

**7. Employee Group Analysis**
- Visualizes the distribution of employees into different company groupings (e.g., Group G, A, B, C) and tracks the count of ungrouped staff.


## Analytics Filter by Name and Range
Users can narrow the analytics data by employee name and date range to analyze a specific person or team over a chosen period.

**Steps to Filter by Name and Range:**
1. Locate the **Name** search field at the top of the Analytics dashboard.
2. Type the employee name or keyword you want to search for.
3. Use the **Date Range** selector to choose a specific **Start Date** and **End Date**.
4. After both filters are selected, the dashboard charts and metrics update to show the matching results.

![Analytics Date Range](/img/hr/analytics/gemini.png)

## Analytics Apply and Clear All
If you want to apply the selected filters or clear them all and return the dashboard to its default view:

**Steps to Use These Controls:**
1. Locate the **Apply** button near the top filter options and click it to load the selected name and date range filters.
2. Locate the **Clear All** button to remove all selected filters at once.
3. After clearing, the dashboard will return to its initial, unfiltered view and show overall company analytics data.

![Analytics Apply and Clear All](/img/hr/analytics/gemini.png)
